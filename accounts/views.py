    ...
    if request.method == "POST":
        ...
        if sudo:
            ...
            if user is not None:
                if user.is_superuser:
                    ...
                    log_activity(
                        ...
                        "LOGIN",
                        level="WARNING",
                        log="Failed sudo login attempt: not authorized to use sudo.",
                    )
                ...
                log_activity(
                    ...
                    "LOGIN",
                    level="WARNING",
                    log=f"Failed sudo login attempt: invalid username ({sudoer}) or password.",
                )
            ...
            if user is not None:
                login(request, user)
                log_activity(
                    user.username, "LOGIN", level="INFO", log=f"Successful login."
                )
                return redirect("tickets:index")
            else:
                messages.error(request, "Invalid username or password.")
                log_activity(
                    ...
                    "LOGIN",
                    level="WARNING",
                    log=f"Failed login attempt: invalid username ({username}) or password.",
                )
...
def admin_board_view(request):
    ...
    if not (user.is_superuser or user.has_perm("accounts.view_activity")):
        log_activity(
            ...
            "UNAUTHORIZED ACCESS",
            level="WARNING",
            log="Unauthorized access to admin board.",
        )
...
def password_change_view(request):
    if request.method == "POST":
        ...
        if user.check_password(old_password):
            if new_password == confirm_password:
                ...
                log_activity(
                    user.username, "UPDATE", level="INFO", log="Password changed."
                )
    ...
    return render(
        request, "accounts/password_change.html", {"form": PasswordChangeForm()}
    )
