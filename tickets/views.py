...
def api_auth_required(view_func):
    ...
    def _wrapped_view(request, *args, **kwargs):
        ...
        return JsonResponse(
            {"error": "Unauthorized. Invalid or missing API key."}, status=401
        )
...
def api_add_comment(request, ticket_id):
    ...
    # Add comment
    ticket.comments.create(author=request.user, comment=request.POST["comment"])
...
def api_ticket_edit(request, ticket_id):
    ...
    if "description" in new_data:
        actions.append(
            f"Description: {ticket.description} -> {request.POST['description']}"
        )
    ...
    if actions:
        ticket.comments.create(author=request.user, comment=";\n".join(actions))
...
def api_list_tickets(request):
    tickets = (
        Ticket.objects.filter(hidden=False)
...
def log_activity(ticket, request_user, log):
    ...
    Changes.objects.create(ticket=ticket, actor=actor, log=log)
                    ...
                    "tickets": [
                        {
                            ...
                            "status": (
                                "new"
                                if last_ticket.status == Ticket.Status.OPEN
                                else "known"
                            ),
                        }
                        for last_ticket in open_bugs
                    ]
                }
            )
...
def view_changes(request):
    ...
    return render(
        request,
        "tickets/changes.html",
        {"changes": changes, "tickets_count": tickets_count},
    )
...
def ticket_detail(request, ticket_id):
    ...
    return render(
        ...
        {"ticket": ticket, "comments": ticket.comments.all().order_by("-created_at")},
    )
...
def search_tickets(request):
    ...
    if User.objects.filter(username__icontains=query).exists():
        all_tickets = Ticket.objects.filter(
            assignee=User.objects.get(username__icontains=query)
        )
    else:
        all_tickets = Ticket.objects.filter(
            title__icontains=query
        ) | Ticket.objects.filter(description__icontains=query)
    if "my" in history:
    ...
    if "closed" in history:
        tickets = tickets.filter(status=Ticket.Status.CLOSED) | tickets.filter(
            status=Ticket.Status.CANCELLED
        )
    else:
        tickets = tickets.exclude(status=Ticket.Status.CLOSED).exclude(
            status=Ticket.Status.CANCELLED
        )
    if tickets.count() == 0:
...
def my_tasks(request):
    ticket_list = (
        Ticket.objects.filter(hidden=False, assignee=request.user)
        .exclude(status=Ticket.Status.CLOSED)
        .exclude(status=Ticket.Status.CANCELLED)
        .order_by("-updated_at")
    )
...
def in_progress_view(request):
    ticket_list = Ticket.objects.filter(
        hidden=False, status=Ticket.Status.IN_PROGRESS
    ).order_by("-updated_at")
    paginator = Paginator(ticket_list, 10)
    tickets_count = ticket_list.count()
    page_number = request.GET.get("page")
...
def edit_ticket(request, ticket_id):
    ...
    if request.method == "POST":
        ...
        if User.objects.get(pk=request.POST["assignee"]).id != ticket.assignee.id:
            try:
                changes.append(
                    f"Assigned: {ticket.assignee} -> {User.objects.get(pk=request.POST['assignee'])}"
                )
            except User.DoesNotExist:
                return JsonResponse({"error": "Invalid assignee"}, status=400)
        ...
        if (
            ...
            and request.POST["due_date"] != ticket_due_date
        ):
        ...
        return render(
            ...
            {"ticket": ticket, "edit_form": TicketForm(instance=ticket)},
        )
...
def new_comment(request, ticket_id):
    ...
    if request.method == "POST":
        ...
        ticket.comments.create(author=request.user, comment=comment)
        messages.success(
            request, f"Comment added successfully to the ticket #{ticket_id}"
        )
        log_activity(ticket, request.user, "Added comment to ticket")
        return redirect("tickets:ticket_detail", ticket_id=ticket_id)
    else:
        return render(
            request,
            "tickets/new_comment.html",
            {"comment_form": CommentForm(), "ticket": ticket},
        )
...
def edit_comment(request, ticket_id, comment_id):
    ...
    if request.method == "POST":
        ...
        comment.save()
        messages.success(
            request, f"Comment updated successfully to ticket #{ticket_id}"
        )
        log_activity(ticket, request.user, "Edited comment in ticket")
        return render(request, "tickets/ticket_detail.html", {"ticket": ticket})
    else:
        return render(
            request, "tickets/edit_comment.html", {"ticket": ticket, "comment": comment}
        )
