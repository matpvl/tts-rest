...
class Ticket(models.Model):
    ...
    issuer = models.ForeignKey(
        "auth.User", on_delete=models.CASCADE, related_name="issued_tickets"
    )
    assignee = models.ForeignKey(
        "auth.User", on_delete=models.CASCADE, related_name="assignee_tickets"
    )
    priority = models.CharField(
        max_length=6, choices=Priority.choices, default=Priority.LOW
    )
    category = models.CharField(
        max_length=7, choices=Category.choices, default=Category.FEATURE
    )
    status = models.CharField(
        max_length=11, choices=Status.choices, default=Status.DRAFT
    )
